...
public class BuildServiceMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		validateParameters();
		injectServiceBuilderDependency();
	...
	private void injectServiceBuilderDependency() {
		boolean alreadyPresent = project.getDependencies().stream()
				.anyMatch(d ->
						"com.easybase".equals(d.getGroupId()) && "easybase-service-builder".equals(d.getArtifactId()));

		if (!alreadyPresent) {
			Dependency dep = new Dependency();
			dep.setGroupId("com.easybase");
			dep.setArtifactId("easybase-service-builder");
			dep.setVersion(pluginVersion);
			dep.setScope("compile");
			project.getDependencies().add(dep);
			getLog().info("EasyBase Service Builder: auto-injected easybase-service-builder:" + pluginVersion);
		}
	}

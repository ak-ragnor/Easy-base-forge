...
import com.easybase.forge.rest.model.HttpMethod;
...
public class BaseControllerGenerator implements RestArtifactGenerator {
	...
	public List<GeneratedArtifact> generate(ApiResource resource, GeneratorConfig config) {
		...
		Path outputPath = PackageUtils.packageToPath(config.getResolvedOutputDirectory(), basePkg)
				.resolve(baseName + ".java");

		return List.of(new GeneratedArtifact(outputPath, RestArtifactType.BASE_CONTROLLER, javaFile.toString()));
	}
	...
	private MethodSpec buildEndpointMethod(
		...
		for (ApiParameter param : endpoint.parameters()) {
			if (param.in() == ParameterLocation.PATH || param.in() == ParameterLocation.QUERY) {
				String javaName = NamingUtils.sanitizeName(param.name());
		...
		if (endpoint.requestBody() != null) {
			...
			String bodyName = NamingUtils.deriveBodyParamName(bodyType);

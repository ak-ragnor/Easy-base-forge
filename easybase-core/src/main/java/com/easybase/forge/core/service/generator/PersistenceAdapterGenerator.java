...
public class PersistenceAdapterGenerator implements ServiceArtifactGenerator {
	...
	public List<GeneratedArtifact> generate(ServiceConfig config) {
		...
		String adapterBasePkg = ServiceGeneratorUtils.persistenceBasePackage(config);
		...
		ServiceGeneratorUtils.applyAuthors(adapterBuilder, config);

		JavaFile javaFile = JavaFile.builder(pkg, adapterBuilder.build())
	...
	private MethodSpec buildConstructor(ServiceConfig config, ClassName jpaRepoType) {
		String jpaRepoParamName = ServiceGeneratorUtils.fieldName(config, "JpaRepository");
		...
		if (config.getHook().isEnabled()) {
			...
			String hooksParamName = ServiceGeneratorUtils.fieldName(config, "Hooks");

...
public class HookBaseGenerator implements ServiceArtifactGenerator {
	...
	public List<GeneratedArtifact> generate(ServiceConfig config) {
		...
		String pkg = ServiceGeneratorUtils.hookBasePackage(config);
		...
		TypeSpec.Builder builder = TypeSpec.interfaceBuilder(hookBaseName)
				...
				.addMethod(buildDefaultVoidMethod(
		...
		TypeSpec hookBase = builder.build();

		JavaFile javaFile =
				JavaFile.builder(pkg, hookBase).skipJavaLangImports(true).build();

		Path outputDir = ServiceGeneratorUtils.packageToPath(config.getResolvedOutputDirectory(), pkg);
		Path outputPath = outputDir.resolve(hookBaseName + ".java");

		return List.of(new GeneratedArtifact(outputPath, ArtifactType.SERVICE_HOOK_BASE, javaFile.toString()));
	}
	...
	private MethodSpec buildDefaultVoidMethod(String name, TypeName paramType, String paramName, String javadoc) {
		return MethodSpec.methodBuilder(name)
				.addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
				.addParameter(paramType, paramName)
				.addJavadoc(javadoc + "\n")

...
public class AnnotationBuilder {
	...
	public static AnnotationSpec build(ValidationConstraint constraint) {
		if (constraint instanceof ValidationConstraint.NotNull) {
			return AnnotationSpec.builder(ClassName.get(JAKARTA_CONSTRAINTS, "NotNull"))
					.build();
		}
		if (constraint instanceof ValidationConstraint.NotBlank) {
			return AnnotationSpec.builder(ClassName.get(JAKARTA_CONSTRAINTS, "NotBlank"))
					.build();
		}
		if (constraint instanceof ValidationConstraint.Email) {
			return AnnotationSpec.builder(ClassName.get(JAKARTA_CONSTRAINTS, "Email"))
					.build();
		}
